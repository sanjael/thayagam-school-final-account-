...
def seed_custom_fees():
    db = next(get_db())
    classes_by_name = {c.name.strip(): c.id for c in db.query(Class).all()}
    print("Found classes:", classes_by_name)

    academic_years = ["2026-2027", "2024-2025"]

    # Delete existing fee structures for these academic years to prevent duplicates
    db.query(FeeStructure).filter(FeeStructure.academic_year.in_(academic_years)).delete(synchronize_session=False)
    db.commit()

    fees_to_add = []

    # Defined Class Fees Mapping
    specific_tuition = {
    ...
    db.commit()
    print(f" Successfully seeded {len(fees_to_add)} fee structure records for {academic_years}!")
