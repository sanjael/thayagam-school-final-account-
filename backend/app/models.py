...
class User(Base):
    ...
    role            = Column(Enum(UserRole, name="user_role"), nullable=False, default=UserRole.accountant)
...
class FeeStructure(Base):
    ...
    id            = Column(Integer, primary_key=True, index=True)
    class_id      = Column(Integer, ForeignKey("classes.id"), nullable=False)
    term          = Column(Enum(Term, name="term_type", values_callable=lambda obj: [e.value for e in obj]), nullable=False)
...
class FeePayment(Base):
    ...
    payment_mode  = Column(Enum(PaymentMode, name="payment_mode"), default=PaymentMode.cash)
...
class SchoolSettings(Base):
    ...
    current_term          = Column(String(20), default="Term 1")
